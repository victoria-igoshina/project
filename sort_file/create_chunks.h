...
size_t CreateChunks(const TFilesNames& filesNames, const size_t maxAmountInChunks) {
    ...
    try {
        ...
        for (TIntegerType i = 0; i < numberChunks + 1; ++i) {
            std::remove(filesNames.GetOutputIterationChunkFile(0, i).c_str());
            std::remove(filesNames.GetOutputIterationChunkCountFile(0, i).c_str());
        }
